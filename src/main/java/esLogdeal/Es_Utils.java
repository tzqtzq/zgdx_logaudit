...
public class Es_Utils {
    ...
    protected static final String INDEX_DEMO_HDFS = "logaudit_hdfs";
    protected static final String INDEX_DEMO_HIVE = "logaudit_hive";
    protected static final String INDEX_DEMO_YARN = "logaudit_yarn";
     ...
     */
    private Es_Utils() {}
    protected static TransportClient startupClient() {
        /**
        ...
        if (client == null) {
            try {
                Settings settings = Settings.builder()
                        .put("cluster.name", "production-es")
                        //自动感知的功能（可以通过当前指定的节点获取所有es节点的信息）
                        .put("client.transport.sniff", true)
                        .build();
                //创建client
                client = new PreBuiltTransportClient(settings).addTransportAddresses(
                        new InetSocketTransportAddress(InetAddress.getByName("10.142.106.84"), 9300),
                        new InetSocketTransportAddress(InetAddress.getByName("10.142.106.85"), 9300),
                        new InetSocketTransportAddress(InetAddress.getByName("10.142.106.86"), 9300));
//                        new InetSocketTransportAddress(InetAddress.getByName("hop-0"), 9300),
//                        new InetSocketTransportAddress(InetAddress.getByName("hop-1"), 9300),
//                        new InetSocketTransportAddress(InetAddress.getByName("hop-2"), 9300));
